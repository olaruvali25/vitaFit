...
const fragmentShader = `
  ...
  void main() {
    ...
    vec4 cppnColor = cppn_fn(uv, 0.1 * sin(0.3 * iTime), 0.1 * sin(0.69 * iTime), 0.1 * sin(0.44 * iTime));

    // Keep dynamic CPPN output but tint toward pale green
    vec3 color = cppnColor.rgb;
    vec3 greenTint = vec3(0.62, 0.90, 0.76);
    color = mix(color, greenTint, 0.35);

    // Subtle dynamic glow to emphasize motion
    float glow = 0.03 * sin(iTime * 0.7) + 0.03;
    color += glow * vec3(0.20, 0.28, 0.22);
...
export default function ShaderBackground() {
  ...
  useGSAP(() => {
    ...
    gsap.set(canvasRef.current, { filter: 'blur(18px)', scale: 1.08, autoAlpha: 0.75 })
    gsap.to(canvasRef.current, { filter: 'blur(0px)', scale: 1, autoAlpha: 1, duration: 1.4, ease: 'power3.out', delay: 0.25 })
  })
  ...
  return (
    ...
    >
      ...
      <div className="pointer-events-none absolute inset-0 bg-gradient-to-t from-[#dff7eb]/20 via-transparent to-transparent" />
    </div>
  )
}
